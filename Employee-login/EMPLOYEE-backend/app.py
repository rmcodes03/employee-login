...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:risabh@localhost:5432/ems'
db = SQLAlchemy(app)
...
class Meeting(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(100), nullable=False)
    meeting_code = db.Column(db.String(100), nullable=False)
    date = db.Column(db.Date, nullable=False)
    time = db.Column(db.String(100), nullable=False)
    attendees = db.Column(db.String(255), nullable=False)
    description = db.Column(db.String(255), nullable=False)

    def __init__(self, title, meeting_code, date, time, attendees, description):
        self.title = title
...
def add_meeting():
    ...
    # Create a new instance of Meeting model
    new_meeting = Meeting(
        title=title,
    ...
    # Add the new meeting to the session and commit the changes
...
def get_meetings():
    # Query the database to fetch all meetings
